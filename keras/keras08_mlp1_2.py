...
# 1번째 방식 tf.reshape(): 텐서의 형태 변환 
# x = x.T # 전치행렬로 변경 numpy에 선언되어 있고 자주사용되어 미리 선언되어 있는듯
x = tf.transpose(x) 

# 2번째 방식 tf.transpose(): 텐서의 행과 열 맞추기
# 5행 2열로 바꿔준다. 즉, 행과 열을 바꿔준다. / x = np.array([[1,6], [2,7], [3,8], [4,9], [5,10]])  
# x = tf.reshape(x, shape=(5, 2))

# x = np.array([[1,6], [2,7], [3,8], [4,9], [5,10]]) # 수동으로 하는건 하지 않는다
